...
sigma = 0.3
Threshold_Hard3D = 1.7*sigma           # Threshold for Hard Thresholding
First_Match_threshold = 200             # 用于计算block之间相似度的阈值
...
def Step1_fast_match(_noisyImg, _BlockPoint):
    ...
    *_BlockPoint:当前block的坐标及大小
    ...
    dct_img = haar2.haar_2d(img.astype(numpy.float64))  # 对目标作block作二维余弦变换
    ...
    for i in range(blk_num):
        for j in range(blk_num):
            ...
            dct_Tem_img = haar2.haar_2d(tem_img.astype(numpy.float64))
            #print(dct_img)
    ...
    *要将_similar_blocks第三维依次取出,然在频域用阈值滤波之后,再作反变换
    ...
    for i in range(m_Shape[1]):
        for j in range(m_Shape[2]):
            tem_Vct_Trans = haar2.haar_wavelet_tranform(_similar_blocks[:, i, j])
            tem_Vct_Trans[numpy.abs(tem_Vct_Trans[:]) < Threshold_Hard3D] = 0.
            statis_nonzero += tem_Vct_Trans.nonzero()[0].size
            _similar_blocks[:, i, j] = haar2.ihaar(tem_Vct_Trans)[0]
...
def Aggregation_hardthreshold(_similar_blocks, blk_positions, m_basic_img, m_wight_img, _nonzero_num, Count, Kaiser):
    ...
    *对于最后的数组，乘以凯撒窗之后再输出
    ...
    for i in range(Count):
        ...
        tem_img = (1./_nonzero_num)*haar2.ihaar_2d(_similar_blocks[i, :, :]) * Kaiser
...
def Step2_fast_match(_Basic_img, _noisyImg, _BlockPoint):
    ...
    *_BlockPoint:当前block的坐标及大小
    ...
    dct_img = haar2.haar_2d(img.astype(numpy.float32))  # 对目标作block作二维余弦变换
    Final_similar_blocks[0, :, :] = dct_img

    n_img = _noisyImg[present_x: present_x+Blk_Size, present_y: present_y+Blk_Size]
    dct_n_img = haar2.haar_2d(n_img.astype(numpy.float32))  # 对目标作block作二维余弦变换
    ...
    for i in range(blk_num):
        for j in range(blk_num):
            ...
            dct_Tem_img = haar2.haar_2d(tem_img.astype(numpy.float32))
            m_Distance = numpy.linalg.norm((dct_img-dct_Tem_img))**2 / (Blk_Size**2)
    ...
    if Count > 0:
        for i in range(1, Count):
            ...
            n_img = _noisyImg[present_x: present_x+Blk_Size, present_y: present_y+Blk_Size]
            Final_noisy_blocks[i, :, :] = haar2.haar_2d(n_img.astype(numpy.float64))
    ...
    *返回的Wiener_wight用于后面Aggregation
    ...
    for i in range(m_Shape[1]):
        for j in range(m_Shape[2]):
            ...
            tem_Vct_Trans = numpy.matrix(haar2.haar_wavelet_tranform(tem_vector))
            Norm_2 = numpy.float64(tem_Vct_Trans * tem_Vct_Trans.T)
            ...
            tem_Vct_Trans = m_weight * haar2.haar_wavelet_tranform(tem_vector)
            _Similar_Bscs[:, i, j] = haar2.ihaar(tem_Vct_Trans)[0]
...
def Aggregation_Wiener(_Similar_Blks, _Wiener_wight, blk_positions, m_basic_img, m_wight_img, Count, Kaiser):
    ...
    *对于最后的数组，乘以凯撒窗之后再输出
    ...
    for i in range(Count):
        point = blk_positions[i, :]
        tem_img = _Wiener_wight * haar2.ihaar_2d(_Similar_Blks[i, :, :]) # * Kaiser
        m_basic_img[point[0]:point[0]+_shape[1], point[1]:point[1]+_shape[2]] += tem_img
        m_wight_img[point[0]:point[0]+_shape[1], point[1]:point[1]+_shape[2]] += block_wight
...
if __name__ == '__main__':
    ...
    img_name = "sigma20.jpg"  # 图像的路径

    img = cv2.resize(cv2.imread(img_name, cv2.IMREAD_GRAYSCALE),(128,128))    # 读入图像，cv2.IMREAD_GRAYSCALE:以灰度模式读入图像
